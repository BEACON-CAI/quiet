...
func ReadIpList(fileName string) (ipList []models.Target) {
	ipListFile, err := os.Open(fileName)
	if err != nil {
		log.Fatalf("Open ip List file err, %v", err)
	}
	...
	for scanner.Scan() {
		...
		if len(tmpPort) == 2 {
			...
			if vars.SupportProtocols[protocol] {
				addr := models.Target{IP: ip, Port: port, Protocol: protocol}
...
func ReadUserDict(userDict string) (users []string, err error) {
	...
	if err != nil {
		log.Fatalf("Open user dict file err, %v", err)
	}
...
func ReadPasswordDict(passDict string) (password []string, err error) {
	...
	if err != nil {
		log.Fatalf("Open password dict file err, %v", err)
	}
